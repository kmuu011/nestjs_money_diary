...
describe('Todo Repository', () => {
    ...
    beforeAll(async () => {
        ...
        for(let i=0 ; i<4 ; i++){
            ...
            const todo: TodoEntity = await todoRepository.selectOne(savedTodoGroupInfo, savedTodoInfo.idx);
    ...
    describe('selectOne()', () => {
        it('할일 상세 조회', async () => {
            const result: TodoEntity = await todoRepository.selectOne(savedTodoGroupInfo, savedTodoInfo.idx);

            expect(result instanceof TodoEntity).toBeTruthy();
        });
    });

    describe('selectList()', () => {
        it('할일 목록 조회', async () => {
            const result: [TodoEntity[], number] = await todoRepository.selectList(savedTodoGroupInfo, 1, 10);

            expect(result[0].every(v => v instanceof TodoEntity)).toBeTruthy();
    ...
    describe('createTodo()', () => {
        it('할일 등록', async () => {
            const result: TodoEntity = await todoRepository.createTodo(getCreateTodoData());
    ...
    describe('updateTodo()', () => {
        it('할일 수정', async () => {
            ...
            const notCompletedOne: TodoEntity
